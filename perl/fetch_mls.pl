...
my $states_arg = $ARGV[0];
my $cities_arg = $ARGV[1];
my $pages_arg  = $ARGV[2];
...
if (defined($states_arg)) {
    my $states_str = make_sql_list($states_arg);
    $states = $dbh->selectall_hashref("select * from state where state in " . $states_str , "state");
} else {
...
foreach my $state (%$states) {
    ...
    if (defined($cities_arg)) {
        my $cities_str = make_sql_list($cities_arg);
        $cities = $dbh->selectall_hashref("select c.id,c.city from city c join state_city sc " .
                                           "where sc.state_id=? and sc.city_id=c.id and c.city in " . $cities_str, "city",
                                           {},$state_id);
    } else {
        $cities = $dbh->selectall_hashref("select c.id,c.city from city c join state_city sc " .
    ...
    $properties->{$state}{total} = 0;
    $properties->{$state}{new} = 0;

    foreach my $city (keys %$cities) {
#    foreach my $city ('Rancho Cordova') {
        my $page_num = 1;
        my $found_pages=0;
        my $city_id = $cities->{$city}{id};

        $properties->{$state}{$city}{total} = 0;
        $properties->{$state}{$city}{new} = 0;

        print "\t**** $city ****\n";

        while (1) {
            my $state_city_page = browse_state_city_page($states->{$state}, $cities->{$city}, $page_num);
            last if (!keys %{$state_city_page->{properties}->[0]});

            print "\t\t==== $city:page$page_num:(". scalar(@{$state_city_page->{properties}}) . ") ====\n";
            # if (scalar(@{$state_city_page->{properties}}) == 1 && !defined($state_city_page->{properties}->[0]->{PropertyPrice})) {
            if (!defined($state_city_page->{properties}->[0]->{PropertyPrice})) {
                # new style
                my @property_ids   = keys (%{$state_city_page->{properties}->[0]});
                last if (scalar(@property_ids) == 0);
                # print join ", ", @property_ids;
                my @property_array = map $state_city_page->{properties}->[0]->{$_}, @property_ids;
                $state_city_page->{properties} = \@property_array;
            } else {
                Carp::croak "Invalid page ";
            }
            for (my $i = 0; $i < scalar(@{$state_city_page->{properties}}); $i++) {
                my $property = $state_city_page->{properties}->[$i];
...
sub collect_data {
    ...
    for (my $j = 0; $j < scalar(@$summary_th_nodes); ++$j) {
        ...
        if ($key eq "Property Features") {
            my @li_nodes = $summary_td_nodes->[$j]->look_down("_tag","li");
            ...
            $property->{$key} = join " ", map {$_->as_trimmed_text} $summary_td_nodes->[$j]->descendents();
        } else {
            $property->{$key} = $val;
        }
    }
...
sub browse_state_city_page($$$) {
    my ($state_obj, $city_obj, $page_num) = @_;
    my $retries = 0;
    my $ret->{properties} = [];
    ...
    while ($retries++ < MAX_RETRIES) {
        $resp=$ua->post("http://www.realtor.com/handlers/resources.ashx", 
                        Content => 
                        {
                         ...
                         PageData =>	'suffix=&status=homes+for+sale&baseurl=realestateandhomes-search&city=' . $city_obj->{city} . '&state=' . $state_obj->{state} . '&statename=CA&type=single-family-home%3Bcondo-townhome-row-home-co-op&listingType=single-family-home%3Bcondo-townhome-row-home-co-op&sby=2&sortby=2&userlocation=' . $city_obj->{city} . '%2C' . $state_obj->{state} . '&MetaKey=srp&seo_loc=City&PageH1=' . $city_obj->{city} . '%2C+' . $state_obj->{state} . '+Real+Estate+and+Homes+for+Sale&TotalProp=105%2C848%2C376&pagesize=50&_rdcLibUrl=http%3A%2F%2Fstatic.move.com%2Flib%2Frdx%2F6.0.06%2F',

                         WidgetList =>	'[{"ID": "currentProperty", "Param": "extras=status&ajax=0&DataAdapterKey=dat&WidgetRelative=%2FcurrentProperty", "Widget": "currentProperty", "WidgetRelative": "/currentProperty", "extraLoad": null}, {"ID": "oldSRP", "Param": "ajax=0&listingTypeDefault1=single-family-home&listingTypeDefault2=condo-townhome-row-home-co-op&statusDefault=sale&sortbyDefault=2&OmnitureUpdate=1&geo_server=geo.svc.move.com&showCustoimzeIndicator=1&srpFadeOutRate=0.5&listingDataSource=oldSRPHeader&DataAdapterKey=dat&ArgAdapterKey=argSRP&cookieName4RecentViewed=RecentViewed&cookieName4RecentSearch=RecentSearch&pageStatus=SRPWidget%7ColdSRP%7ColdSRPHeader&defaultSRPTab=oldSRPTab&statusWidget=oldFacetSearch&FacetLimit=20&extras=mlslid%7Cpfbm%7Ccity%7Cstate&WidgetRelative=%2FoldSRP", "Widget": "oldSRP", "WidgetRelative": "/oldSRP", "extraLoad": null}]'
                        }
                       );
    ...
    if (!$resp->is_success) {
        Carp::croak "could not get summary page for $state_obj->{state}, $city_obj->{city}, $page_num";
    }

    my $ind = index($resp->content, "flexui.initWidget;flexui.dataSet.byKey('SRP', ");
    if ($ind == -1) {
        Carp::croak "unexpected page format found 1 for $state_obj->{state}, $city_obj->{city}, $page_num";
    }
    my $begin_relevant = substr($resp->content, $ind + length("flexui.initWidget;flexui.dataSet.byKey('SRP', "));
...
sub insert_property {
    ...
    $sth = $dbh->prepare("insert into " .
                         ...
                         "zip=values(zip),hashdump=values(hashdump)");
